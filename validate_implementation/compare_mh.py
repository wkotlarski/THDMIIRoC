...
def get_spectrum(x, idx):
    ...
    print(f'index: {idx}')
    print(f'mhG: {x[6-offset]}, mhW: {spectrum.blocks["MASS"][25]}, diff (%): {x[51-offset]-spectrum.blocks["MASS"][25]}')
    ...
    print(f'Higgs mass: {x[7]}')
    print(f'boundary mass: {x[5]}')
    if abs(x[51-offset]-spectrum.blocks["MASS"][25]) > worst_diff:
        worst_diff = abs(x[51-offset]-spectrum.blocks["MASS"][25])
        worst_idx = idx
