...
best_mh = 10000
nr_pval_1 = 0
def get_spectrum(x, idx, debugPrint=False):
    global nr_pval_1
    global worst_diff
    global worst_idx
    global hs
    ...
    offset = 0
    slha_input.blocks['MINPAR'][1] = 1.85 #x[4]
    ...
    if abs(spectrum.blocks["MASS"][6]-172.57) < abs(best_mt-172.57):
       best_mt = spectrum.blocks["MASS"][6]

    print(f'{spectrum.blocks["HIGGSSIGNALS"][4]}, {best_pval}, {spectrum.blocks["HIGGSSIGNALS"][4]==best_pval}, {spectrum.blocks["MASS"][25]}')

    if ((spectrum.blocks["HIGGSSIGNALS"][4] > best_pval) or (abs(spectrum.blocks["HIGGSSIGNALS"][4] == best_pval) and abs(spectrum.blocks["MASS"][25]-125.20) < abs(best_mh-125.2))):
       best_idx = idx
       best_mh = spectrum.blocks["MASS"][25]
       best_pval = spectrum.blocks["HIGGSSIGNALS"][4]

    if spectrum.blocks["HIGGSSIGNALS"][4] == 1:
        nr_pval_1 = nr_pval_1+1
...
get_spectrum(csvfile.to_numpy()[best_idx], -1, True)
print('best point')
print(f'best: {best_pval}')
print(f'best idx {best_idx}')
print(f'best mt: {best_mt}')
print(f'best mh: {best_mh}')
print(f'nr of p-val = 1: {nr_pval_1}')
