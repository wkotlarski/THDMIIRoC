...
def get_spectrum(x, idx):
    ...
    slha_input.blocks['MINPAR'][3] = x[18]
    for i in range(7):
       slha_input.blocks['MINPAR'][4+i] = x[11+i]
    slha_input.blocks['MINPAR'][11] = x[26]
    for i in range(7):
       slha_input.blocks['MINPAR'][12+i] = x[19+i]
    slha_input.blocks['MINPAR'][19] = x[34]
    for i in range(7):
       slha_input.blocks['MINPAR'][20+i] = x[27+i]
    ...
    print(f'mhG: {x[6]}, mhW: {spectrum.blocks["MASS"][25]}, diff (%): {100*(1-x[51]/spectrum.blocks["MASS"][25])}')

    print(f'l1 high GP: {x[43]}')
    print(f'l2 high GP: {x[44]}')
    print(f'l3 high GP: {x[45]}')
    print(f'l4 high GP: {x[46]}')
    print(f'l5 high GP: {x[47]}')
    print(f'Yt high GP: {x[50]}')
    print(f'l1 low GP: {x[35]}')
    print(f'l2 low GP: {x[36]}')
    print(f'l3 low GP: {x[37]}')
    print(f'l4 low GP: {x[38]}')
    print(f'l5 low GP: {x[39]}')
    print(f'Yt low GP: {x[40]}')
    print(f'l1: {x[35]/spectrum.blocks["HMIX"][31]}')
    print(f'l2: {x[36]/spectrum.blocks["HMIX"][32]}')
    print(f'l3: {x[37]/spectrum.blocks["HMIX"][33]}')
    print(f'l4: {x[38]/spectrum.blocks["HMIX"][34]}')
    print(f'l5: {x[39]/spectrum.blocks["HMIX"][35]}')
    print(proc.stdout.decode())
    res = 100*(1-x[6]/spectrum.blocks["MASS"][25])
    if res > worst_diff:
        worst_diff = res
        worst_idx = idx
    return res
...
#   data.append(get_spectrum(row, idx))

print(worst_diff)
print(worst_idx)
data.append(get_spectrum(csvfile.to_numpy()[442], 1))
#data.append(get_spectrum(csvfile.to_numpy()[0], 1))

plt.rcParams['text.usetex'] = True
plt.hist(data, bins=30, color='skyblue', edgecolor='black')
plt.xlabel(r'$\Delta m_h (\%)$')
plt.show()
